...
function! LoadPluginScript ()
  ...
  if has("gui_running")
    ...
    if exists(':ColorSchemeSave')
      ...
      if filereadable(expand("$HOME/.vim/colors/".mycolorscheme."_term.vim")) ||
              \    filereadable(expand("$HOME/vimfiles/colors/".mycolorscheme."_term.vim"))
        silent !echo "using colorsupport.vim with ".mycolorscheme."_term"
        execute 'colorscheme '.mycolorscheme.'_term'
      else
